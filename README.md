...
5. **深度分析**：對每篇產出有觀點、有態度的分析文章 — 不是學術摘要，是帶批判視角的評析
...
> 本文提出了一種基於 X 的方法，在 benchmark 上取得 SOTA，證明了 Y 的有效性。

而是帶敘事弧度、有立場的分析：

- 不用被動語態（禁「本文提出了...」）
- 每個章節至少一個引號洞察（`> 「精煉觀察」`）
- 章節標題是問題而非描述（「為什麼 X 視角重要？」而非「方法介紹」）
- 限制段落要具體（「驗證只限 X 場景，因為 Y」而非「需要更多實驗」）
- 結尾有個人定位和一句話總結
